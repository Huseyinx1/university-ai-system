...
with tab1:
    ...
    with col1:
        ...
        prompt = st.chat_input("Bir şeyler sor (Örn: Servis kaçta? veya Bana bir fıkra anlat)...")
    ...
    if prompt:
        ...
        # Ekranı Yenile (Kullanıcı mesajı görünsün diye)
        st.rerun() 

    # Eğer son mesaj kullanıcıdan geldiyse, AI Cevap Versin
    if st.session_state["mesajlar"][-1]["role"] == "user":
        ...
        # Oto-Scroll
        components.html("""<script>var elements=window.parent.document.querySelectorAll('.stChatMessage');if(elements.length>0){elements[elements.length-1].scrollIntoView({behavior:"smooth",block:"end"});}</script>""", height=0)


# --- TAB 2: TAKVİM (Önceki Detaylı Hali) ---s
with tab2:
